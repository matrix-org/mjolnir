    ...
    try {
        if (parts.length === 1 || parts[1] === 'status') {
            ...
            const menu = "" +
                ...
                "!mjolnir since <date>/<duration> <action> <limit> [rooms...] [reason] - Apply an action (kick, ban, mute or just show) to all users who joined a room since a given date (up to <limit> users)\n" +
