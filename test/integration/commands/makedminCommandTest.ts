...
describe("Test: The make admin command", function () {
    ...
    it("Mjölnir make the bot self room administrator", async function () {
        ...
        LogService.debug(
            "makeadminTest",
            `moderator creating targetRoom: ${targetRoom}; and inviting ${mjolnirUserId}`,
        );
        ...
        try {
            ...
            assert.notEqual(
                powerLevels["users"][mjolnirUserId],
                100,
            ...
            await getFirstReaction(mjolnir, this.mjolnir.managementRoomId, "✅", async () => {
                ...
                return await moderator.sendMessage(this.mjolnir.managementRoomId, {
                    msgtype: "m.text",
                    body: `!mjolnir make admin ${targetRoom}`,
                });
            });
        } finally {
    ...
    it("Mjölnir make the tester room administrator", async function () {
        ...
        LogService.debug(
            "makeadminTest",
            `User A creating targetRoom: ${targetRoom}; and inviting ${userBId} and ${userCId}`,
        );
        try {
            await userB.start();
            await userC.start();
        ...
        try {
            ...
            await getFirstReaction(mjolnir, this.mjolnir.managementRoomId, "✅", async () => {
                ...
                return await moderator.sendMessage(this.mjolnir.managementRoomId, {
                    msgtype: "m.text",
                    body: `!mjolnir make admin ${targetRoom} ${userBId}`,
                });
            });
        } finally {
            await moderator.stop();
