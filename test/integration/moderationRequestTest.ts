...
describe("Test: Requesting moderation", async () => {
    it(`Mjölnir can setup a room for moderation requests using !mjolnir command`, async function () {
        ...
        let roomId = await goodUser.createRoom({
            invite: [await badUser.getUserId(), await this.mjolnir.client.getUserId()],
        });
    ...
    it(`Mjölnir propagates moderation requests`, async function () {
        ...
        await sendReport({
            eventId: badEventId,
            nature: "org.matrix.msc3215.abuse.nature.disagreement",
            text: badText,
        });
        // With a comment.
        await sendReport({
            eventId: badEventId2,
            nature: "org.matrix.msc3215.abuse.nature.toxic",
            text: badText2,
        ...
        await sendReport({
            eventId: badEventId4,
            nature: "org.matrix.msc3215.abuse.nature.spam",
            textPrefix: badText4.substring(0, 256),
        });
        // With a very long text.
        await sendReport({
            eventId: badEventId5,
            nature: "org.matrix.msc3215.abuse.nature.other",
        ...
        for (let toFind of reportsToFind) {
            for (let event of notices) {
                if ("content" in event && "body" in event.content) {
                    ...
                    assert.equal(
                        matches.get("nature")!.groups!.natureSource,
                        toFind.nature,
                        "The report should specify the correct nature",
                    );
                    found.push(toFind);
                    break;
                }
